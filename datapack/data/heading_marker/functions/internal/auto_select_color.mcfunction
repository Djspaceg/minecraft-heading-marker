...
# Try current next color
scoreboard players operation @s hm.input.color = @s hm.nextcolor

# Check if this color is already used, if so try next ones
...
execute if score @s hm.input.color matches 4 if score @s hm.purple.active matches 1 run scoreboard players set @s hm.input.color 0
