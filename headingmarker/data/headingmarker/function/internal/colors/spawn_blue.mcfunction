...
# Summon armor stand with blue waypoint tags
function headingmarker:internal/colors/summon_blue_macro with storage headingmarker:temp

# Mark as active
scoreboard players set @s hm.blue.active 1

# Configure waypoint properties
execute as @e[type=armor_stand,tag=hm.waypoint.new] run function headingmarker:internal/colors/config/blue

# Ensure the player activates the waypoint for their HUD (apply as player to the nearest configured waypoint)
waypoint modify @e[type=armor_stand,tag=hm.waypoint,limit=1,sort=nearest] color 255

# Confirm
tellraw @s [{"text":"🔵 Blue waypoint set at ","color":"green"},{"score":{"name":"@s","objective":"hm.input.x"}},{"text":" "},{"score":{"name":"@s","objective":"hm.input.y"}},{"text":" "},{"score":{"name":"@s","objective":"hm.input.z"}}]
