...
# Summon armor stand with purple waypoint tags
function headingmarker:internal/colors/summon_purple_macro with storage headingmarker:temp

# Mark as active
scoreboard players set @s hm.purple.active 1

# Configure waypoint properties
execute as @e[type=armor_stand,tag=hm.waypoint.new] run function headingmarker:internal/colors/config/purple

# Ensure the player activates the waypoint for their HUD (apply as player to the nearest configured waypoint)
waypoint modify @e[type=armor_stand,tag=hm.waypoint,limit=1,sort=nearest] color 8388736

# Confirm
tellraw @s [{"text":"🟣 Purple waypoint set at ","color":"green"},{"score":{"name":"@s","objective":"hm.input.x"}},{"text":" "},{"score":{"name":"@s","objective":"hm.input.y"}},{"text":" "},{"score":{"name":"@s","objective":"hm.input.z"}}]
